...
export function NotificationProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const fetchedNotifications = snapshot.docs.map(doc => {
        ...
        const titleText = data.type === "predator" || data.predator ? `Predator Alert: ${data.predator || "Unknown"}` : "System Alert";
  ...
  const addNotification = (notification) => {
    ...
    console.log("Adding local notification is mocked when using Firebase sync", notification);
  };
  ...
  const deleteNotification = (id) => {
    ...
    console.log("Delete notification not fully implemented with Firebase sync", id);
  };
